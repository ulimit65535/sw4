...
def wuqiguo(hwnd, x, y):
    wuqiguo_img_color = cv2.cvtColor(cv2.imread("src/wuqiguo.png"), cv2.IMREAD_COLOR)
    duihua_putonglixian_img = cv2.cvtColor(cv2.imread("src/duihua_putonglixian.png"), cv2.COLOR_BGR2GRAY)
    duihua_tiaozhan_wuqiguo_img = cv2.cvtColor(cv2.imread("src/duihua_tiaozhan_wuqiguo.png"), cv2.COLOR_BGR2GRAY)
    duihua_qianwangxiayiguan_img = cv2.cvtColor(cv2.imread("src/duihua_qianwangxiayiguan.png"), cv2.COLOR_BGR2GRAY)

    lower = np.array([20, 150, 140], dtype="uint8")
    upper = np.array([80, 240, 255], dtype="uint8")

    renwu_wuqiguo_img_color = cv2.cvtColor(cv2.imread("src/renwu_wuqiguo.png"), cv2.IMREAD_COLOR)
    mask = cv2.inRange(renwu_wuqiguo_img_color, lower, upper)
    renwu_wuqiguo_img_color = cv2.bitwise_and(renwu_wuqiguo_img_color, renwu_wuqiguo_img_color, mask=mask)
    renwu_wuqiguo_img = cv2.cvtColor(renwu_wuqiguo_img_color, cv2.COLOR_BGR2GRAY)
    ...
    while True:
        ...
        points = get_match_points(opponent_img, renwu_wuqiguo_img, threshold=0.7)
        if points:
            # 点击任务栏任务
            px, py = points[0]
